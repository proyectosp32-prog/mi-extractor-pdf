...
st.sidebar.markdown("---")
st.sidebar.subheader("Opciones de Agrupación")
...
# --- ESTE ES EL NUEVO INTERRUPTOR ---
desglosar_medidas = st.sidebar.checkbox("Desglosar por medidas/color (No sumar si son diferentes)", value=True)
st.sidebar.caption("Si lo marcas, 'Banco Naga 200cm' y 'Banco Naga 300cm' saldrán separados.")
...
if archivos_subidos:
    ...
    if datos_brutos:
        ...
        if agrupar:
            ...
            if desglosar_medidas:
                # Agrupa por TODO (Producto + Medidas + Color) -> Salen separados
                columnas_agrupar = [c for c in df.columns if c != 'Cantidad']
            else:
                # Agrupa SOLO por nombre -> Se suma todo lo que se llame igual
                columnas_agrupar = ['Producto']
        ...
        st.warning("No se ha encontrado nada con esos filtros.")
